# ============================================================================
# ANALYSIS/CHAT_ROUTES.PY — Conversational AI Review API Endpoints
# ============================================================================
...
from app.analysis.chat_models import (
    ChatHistoryResponse,
    ChatMessage,
    ChatMessageRequest,
    ChatMessageResponse,
...
from app.analysis.chat_service import SESSION_TTL, chat_service
...
async def start_chat_session(request: ChatStartRequest, db: Session = Depends(get_db)):
    ...
    Provide either:
    - analysis_id: ID of a saved analysis in the database
    - context: Raw analysis result dict (from quick analysis)

    Returns a session_id to use for follow-up messages.
    """
    analysis_data = None

    # Option 1: Load from database by analysis_id
    if request.analysis_id:
        analysis = db.query(Analysis).filter(Analysis.id == request.analysis_id).first()
        if not analysis:
            raise HTTPException(status_code=404, detail="Analysis not found")

        # Build context from the saved analysis
    ...
    # Option 2: Use raw context provided directly
...
async def send_chat_message(request: ChatMessageRequest):
    ...
    The AI has full context of the original analysis and all previous
    messages in this session. Ask about scores, recommendations,
    specific code patterns, how to fix issues, etc.
    """
    try:
...
async def get_chat_history(session_id: str):
    ...
    Returns all messages exchanged in this session along with
    the original analysis summary for context.
    """
    try:
