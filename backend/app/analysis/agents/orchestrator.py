...
from app.analysis.agents.architecture_agent import architecture_agent
from app.analysis.agents.performance_agent import performance_agent
from app.analysis.agents.security_agent import security_agent
...
class AgentOrchestrator:
    ...
    def _build_code_context(
        ...
        This is the same information the single generalist prompt used,
        but each agent will interpret it through their specialist lens.
        """
...
{rag_section}"""
    ...
    def _merge_agent_results(
        ...
        Takes the best insights from each agent and combines them into
        the same JSON structure that the frontend/existing endpoints expect.
        """
        ...
        # Build the unified report (same shape as the single-agent analysis)
        return {
            # Summary — combine all agent summaries
            "summary": self._build_unified_summary(
                security_result, performance_result, architecture_result
            ),

            # Scores — each agent owns its domain score
            ...
            # Full analysis from each agent (the deep specialist insights)
            ...
            # Static analysis data (same as before)
            "ast_analysis": {
            ...
            # NEW: Individual agent reports (deep specialist insights)
            "agent_reports": {
                "security": {
                    "agent": "SecurityAgent",
    ...
    # ====================================================================
    # HELPER METHODS — Score calculation, risk merging, summary building
    # ====================================================================
    ...
    def _calculate_overall_score(
        ...
        """Calculate overall score (1-10) as a weighted average of agent scores.
        ...
        # Weighted average on 0-100 scale, then convert to 1-10
    ...
    def _merge_recommendations(
        ...
        return recommendations[:6]  # Cap at 6 recommendations total
    ...
    async def run_multi_agent_analysis(
        ...
        Pipeline: Static Analysis → RAG → 3 Agents (parallel) → Merge → Store in RAG
        """
        ...
        # Step 3: Build shared code context
    ...
    async def stream_multi_agent_analysis(
        ...
        yield {
            ...
            "data": {
                "step": "agents_running",
                "message": "🔒 SecurityAgent + ⚡ PerformanceAgent + 🏗️ ArchitectureAgent analyzing...",
                "progress": 55,
            },
        }
        ...
        for result in agent_results:
            ...
            yield {
                ...
                "data": {
                    ...
                    "message": f"{status} {result.get('_agent', 'Unknown')} completed",
                    "progress": 75,
                },
            }
